...
PNG grayscale(PNG image)
{
  ...
  for (unsigned x = 0; x < image.width(); x++)
  {
    for (unsigned y = 0; y < image.height(); y++)
    {
      ...
      double gray_value = 0.0; // this will be changed once you determine what the adjusted RGB values will be below!
      double adjusted_red = 0.229 * pixel->r;
      double adjusted_green = 0.587 * pixel->g;
      double adjusted_blue = 0.114 * pixel->b;
      gray_value = adjusted_red + adjusted_green + adjusted_blue;
      pixel->r = (unsigned char) gray_value;
      pixel->g = (unsigned char) gray_value;
...
PNG createSpotlight(PNG image, int centerX, int centerY)
{
  for (unsigned x = 0; x < image.width(); x++)
  {
    ...
    {
      RGBAPixel *pixel = image.getPixel(x, y);
      unsigned distX_toCenX = x - centerX;
      unsigned distY_toCenY = y - centerY;
      unsigned pixel_away = sqrt((distX_toCenX * distX_toCenX) + (distY_toCenY * distY_toCenY));
      if (pixel_away <= 200)
      {
        pixel->r = pixel->r * (1 - 0.005 * pixel_away);
        pixel->g = pixel->g * (1 - 0.005 * pixel_away);
        pixel->b = pixel->b * (1 - 0.005 * pixel_away);
      }
      else if (pixel_away > 200)
      {
        pixel->r = 0;
        pixel->g = 0;
        pixel->b = 0;
...
PNG ubcify(PNG image)
{
  ...
  for (unsigned x = 0; x < image.width(); x++)
  {
    ...
    {
      RGBAPixel *pixel = image.getPixel(x, y);
      double colordist_yellow = colordist(*pixel, *ubc_yellow);
      double colordist_blue = colordist(*pixel, *ubc_blue);
      if (colordist_yellow <= colordist_blue)
      {
        pixel->r = 247;
        pixel->g = 184;
        pixel->b = 0;
      }
      else if (colordist_yellow > colordist_blue)
      {
        pixel->r = 12;
        pixel->g = 35;
        pixel->b = 68;
...
PNG watermark(PNG firstImage, PNG secondImage)
{
  ...
  for (unsigned int x = 0; x < secondImage.width(); x++)
  {
    ...
    {
      RGBAPixel *pixel_firstImage = firstImage.getPixel(x, y);
